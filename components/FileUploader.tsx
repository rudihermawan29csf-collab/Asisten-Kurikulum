...
const FileUploader: React.FC<FileUploaderProps> = ({ onFilesSelected, disabled }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Fix: Explicitly type the result of Array.from as File[] to avoid 'unknown' type errors on line 22, 23, and 28
    const filePromises = (Array.from(files) as File[]).map(file => {
      return new Promise<FileData>((resolve, reject) => {
        ...
        // Fix: 'file' is now correctly recognized as a File (which inherits from Blob)
